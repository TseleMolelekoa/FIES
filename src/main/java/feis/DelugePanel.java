...
public class DelugePanel extends JPanel {
    ...
    private static final String[] ITEMS = {
        "Any leaks visible","Pressure Gauge @ 7 to 10 Bar",
        "Main Valve open","Main Valve closed","Lockout available",
        "Accessibility","Visibility","Signage in place",
        "Signage and Fire system unit clean"
    };
    private final Map<String,JComboBox<String>> checks = new LinkedHashMap<>();

    public DelugePanel(List<Map<String,String>> store, Runnable refreshRecords) {
        this.store = store;
        setLayout(new BorderLayout(8,8));
        setBackground(Theme.BG);
        setBorder(new EmptyBorder(12,14,12,14));
        add(UI.sectionTitle("Fire Deluge System Checklist  —  Fire Equipment Inspection System"), BorderLayout.NORTH);
        ...
        UI.addRowWithScan(card, g, row++, "Equipment No", eqNoField, () ->
            new QRScannerDialog(SwingUtilities.getWindowAncestor(this),
                "Scan Equipment Location Tag", result -> {
    ...
    private JPanel buildCondPanel() {
        ...
        JLabel title = new JLabel("Scan Deluge System Condition Tag");
        title.setFont(Theme.FONT_HEADING); title.setForeground(Theme.PRIMARY);
        JLabel hint  = new JLabel("Scan the QR on the deluge unit to verify pressure/valve status");
        hint.setFont(Theme.FONT_SMALL);  hint.setForeground(Theme.TEXT_SECONDARY);
        JButton btn  = new JButton("Scan");
        btn.setBackground(Theme.PRIMARY_LIGHT); btn.setForeground(Color.WHITE);
        btn.setFont(Theme.FONT_HEADING); btn.setFocusPainted(false);
        btn.setBorder(new EmptyBorder(6,14,6,14));
        btn.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
        btn.addActionListener(e ->
            new QRScannerDialog(SwingUtilities.getWindowAncestor(this),
                "Scan Deluge Condition QR", result -> {
                    hint.setText("✅ Scanned: " + result);
                    hint.setForeground(Theme.SUCCESS);
                    commentsField.setText((commentsField.getText().isEmpty()?"":"  |  ") +
                        "Condition QR: " + result);
                    // Parse pressure value if encoded
                    if (result.contains("PRESSURE:")) {
                        String pStr = extractField(result.replace("|","&"), "PRESSURE");
                        try {
                            double p2 = Double.parseDouble(pStr);
                            checks.get("Pressure Gauge @ 7 to 10 Bar")
                                  .setSelectedItem(p2 >= 7 && p2 <= 10 ? "Yes" : "No");
    ...
    private void save(Runnable refresh) {
        if (dateField.getText().trim().isEmpty()) {
            JOptionPane.showMessageDialog(this,"Date is required.","Validation",JOptionPane.WARNING_MESSAGE);
            return;
        }
        Map<String,String> rec = new LinkedHashMap<>();
        rec.put("Type","Fire Deluge System");
        ...
        store.add(rec);
        refresh.run();
        JOptionPane.showMessageDialog(this,"✅  Deluge System record saved!","Saved",JOptionPane.INFORMATION_MESSAGE);
    }

    private void clear() {
        dateField.setText(new java.text.SimpleDateFormat("yyyy-MM-dd").format(new java.util.Date()));
        areaField.setText(""); deptField.setText(""); eqNoField.setText("");
        locationField.setText(""); commentsField.setText(""); inspectedBy.setText("");
        areaResp.setText(""); emsSupervisor.setText("");
